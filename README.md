...
| 11 | [Day 11 — 异步（二）：fetch 与真实数据](./Day%2011%20%E2%80%94%20%E5%BC%82%E6%AD%A5%EF%BC%88%E4%BA%8C%EF%BC%89%EF%BC%9Afetch%20%E4%B8%8E%E7%9C%9F%E5%AE%9E%E6%95%B0%E6%8D%AE.md) | ⚠️ **`fetch` 不因 404/500 而失败** · 三层错误与健壮封装 · `204` 的坑 · `AbortController` · **竞态条件** · CORS 与 Vite proxy |
| 12 | [Day 12 — 类、原型（认得出就行）](./Day%2012%20%E2%80%94%20%E7%B1%BB%E3%80%81%E5%8E%9F%E5%9E%8B%EF%BC%88%E8%AE%A4%E5%BE%97%E5%87%BA%E5%B0%B1%E8%A1%8C%EF%BC%89.md) | `class` 与 C# 的六个差别 · **`Error` 继承必须设 `this.name`** · 原型链 · **⭐ React 为何抛弃 class 组件** |
| 13 | [Day 13 — 浏览器 API 与 DOM（够用即止）](./Day%2013%20%E2%80%94%20%E6%B5%8F%E8%A7%88%E5%99%A8%20API%20%E4%B8%8E%20DOM%EF%BC%88%E5%A4%9F%E7%94%A8%E5%8D%B3%E6%AD%A2%EF%BC%89.md) | `e.target` vs `e.currentTarget` · `preventDefault` · **WebForm→React 迁移对照表** · `localStorage` 必须 `try/catch` · **`FormData` 别设 `Content-Type`** · `Blob` 下载 |
| 14 | [Day 14 — 收口 + 杂项 + 阶段项目](./Day%2014%20%E2%80%94%20%E6%94%B6%E5%8F%A3%20%2B%20%E6%9D%82%E9%A1%B9%20%2B%20%E9%98%B6%E6%AE%B5%E9%A1%B9%E7%9B%AE.md) | **`Date` 六个坑**（含只在中国时区暴露的那个）· **`Intl` 是 `String.Format` 的替代品** · 正则校验 · **⭐ 阶段项目** |

> 教程中的所有数值输出均经 Node.js 24 实测核对；日期章节在 `TZ=Asia/Shanghai` 下验证；
> Day 11 的 `fetch` 行为通过真实本地 HTTP 服务器验证。
>
> **Day 1–14 为阶段 1（JavaScript 核心），已全部完成。**
